...
void ActivityLocationManager::init(NetworkManager &net,
                                   const std::filesystem::path &cacheDir) {
  cacheDir_ = cacheDir;
  net_ = &net;
  loadApiCache();

  // Check for a pre-seeded summitslist.csv in configDir or cwd before hitting
  // the network
    ...
    if (std::filesystem::exists(p, ec)) {
      ...
      if (ifs) {
        ...
        WorkerService::getInstance().submitTask(
            [this, data = std::move(data)]() { parseSOTA(data); });
      }
      break;
    }
  }
...
bool ActivityLocationManager::getPOTALocation(const std::string &ref,
                                              float &lat, float &lon) {
  ...
  if (it != potaParks_.end() &&
      std::strcmp(it->reference, target.reference) == 0) {
    lat = it->lat;
    lon = it->lon;
    return true;
  }
...
bool ActivityLocationManager::getSOTALocation(const std::string &ref,
                                              float &lat, float &lon) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (!sotaSummits_.empty()) {
    ...
    auto it =
        std::lower_bound(sotaSummits_.begin(), sotaSummits_.end(), target);
    if (it != sotaSummits_.end() &&
        std::strcmp(it->reference, target.reference) == 0) {
      lat = it->lat;
      lon = it->lon;
      return true;
...
void ActivityLocationManager::fetchAndLoad(NetworkManager &net) {
  ...
  net.fetchAsync(
      ...
      [this](std::string data) {
        ...
        WorkerService::getInstance().submitTask(
            [this, data = std::move(data)]() { parsePOTA(data); });
      },
...
void ActivityLocationManager::resolveSummitAsync(const std::string &ref) {
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (sotaApiCache_.count(ref) || sotaApiInFlight_.count(ref))
      return;
      ...
      [this, ref](std::string data) {
        ...
        try {
          ...
          if (j.contains("latitude")) {
            if (j["latitude"].is_number())
              lat = j["latitude"].get<float>();
            else if (j["latitude"].is_string())
              lat = StringUtils::safe_stof(j["latitude"].get<std::string>());
          }
          if (j.contains("longitude")) {
            if (j["longitude"].is_number())
              lon = j["longitude"].get<float>();
            else if (j["longitude"].is_string())
              lon = StringUtils::safe_stof(j["longitude"].get<std::string>());
          }
        } catch (...) {
          LOG_E("ActivityLoc", "Failed to parse SOTA JSON for {}", ref);
        }
