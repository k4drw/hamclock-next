...
OrbitPredictor::~OrbitPredictor() {
  ...
  if (observer_)
    predict_destroy_observer(observer_);
}

void OrbitPredictor::setObserver(double latDeg, double lonDeg,
                                 double altMeters) {
  if (observer_)
    predict_destroy_observer(observer_);
  observer_ = predict_create_observer("QTH", latDeg * kDeg2Rad,
                                      lonDeg * kDeg2Rad, altMeters);
  if (!observer_) {
...
SatPass OrbitPredictor::nextPassAfter(std::time_t utc) const {
  ...
  struct predict_observation los =
      predict_next_los(observer_, elements_, aos.time);
  ...
  struct predict_observation maxEl =
      predict_at_max_elevation(observer_, elements_, aos.time);
...
std::vector<GroundTrackPoint> OrbitPredictor::groundTrack(std::time_t startUtc,
  ...
  for (int s = 0; s < totalSec; s += stepSec) {
    ...
    track.push_back(GroundTrackPoint(lat, lon));
  }
...
double OrbitPredictor::tleAgeDays() const {
  ...
  epoch_tm.tm_isdst = 0;
  std::time_t jan1 = Astronomy::portable_timegm(&epoch_tm);

  double epochSec =
      static_cast<double>(jan1) + (elements_->epoch_day - 1.0) * 86400.0;
