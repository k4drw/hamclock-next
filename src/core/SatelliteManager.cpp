...
SatelliteManager::SatelliteManager(NetworkManager& net)
    : net_(net) {}
...
void SatelliteManager::fetch(bool force)
{
    ...
    if (!force && dataValid_) {
        ...
        if (elapsed < std::chrono::hours(24)) return;
    }

    std::fprintf(stderr, "SatelliteManager: fetching TLE data from celestrak...\n");

    net_.fetchAsync(TLE_URL, [this](std::string response) {
        if (response.empty()) {
            std::fprintf(stderr, "SatelliteManager: fetch failed (empty response)\n");
            return;
        }
        parse(response);
    });
...
const SatelliteTLE* SatelliteManager::findByName(const std::string& search) const
{
    std::lock_guard<std::mutex> lock(mutex_);
