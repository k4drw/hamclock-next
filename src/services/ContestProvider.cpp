...
void ContestProvider::fetch() {
  net_.fetchAsync(CONTEST_URL, [](std::string body) {
    ...
    WorkerService::getInstance().submitTask([body]() {
      ContestData *update = new ContestData();
      size_t pos = 0;
      ...
      while ((pos = body.find("<item>", pos)) != std::string::npos) {
        ...
        update->contests.push_back(c);
      }

      update->lastUpdate = std::chrono::system_clock::now();
      update->valid = !update->contests.empty();

      SDL_Event event;
      SDL_zero(event);
      event.type = HamClock::AE_BASE_EVENT + HamClock::AE_CONTEST_DATA_READY;
