...
// Sentinel value 999 means "use system local time (std::localtime)" — DST-aware.
...
const int ClockAuxPanel::kPresetCount =
    static_cast<int>(sizeof(ClockAuxPanel::kPresets) / sizeof(ClockAuxPanel::kPresets[0]));
...
void ClockAuxPanel::render(SDL_Renderer *renderer) {
  ...
  cat->drawText(renderer, title, x_ + 10, y_ + 5, {0, 200, 255, 255},
                FontStyle::MicroBold);
  ...
  if (height_ > 100) {
    cat->drawText(renderer, "tap to set TZ", centerX, y_ + height_ - 10,
                  {60, 60, 80, 255}, FontStyle::Tiny, true);
  }
}
...
void ClockAuxPanel::recalcMenuLayout() {
  ...
  constexpr int customSectionH = 8 + fieldH + 6 + fieldH + 6; // top-pad + offset + gap + label + bottom-pad
  constexpr int btnH     = 30;
  constexpr int pad      = 8;
  constexpr int mH = titleH + localRowH + (kTotalRows - 1) * rowH + pad + customSectionH + pad + btnH + pad;
...
void ClockAuxPanel::renderMenu(SDL_Renderer *renderer) {
  ...
  // Popup background — match SDOPanel: semi-transparent dark, blend mode
  ...
  cat->drawText(renderer, "Select Timezone",
                menuRect_.x + menuRect_.w / 2, menuRect_.y + 5,
                themes.textDim, FontStyle::SmallRegular, true);

  // Rows: 8 presets + "Custom..."
  for (int i = 0; i < kTotalRows; ++i) {
    const SDL_Rect &r = rowRects_[i];
    bool selected = (i < kPresetCount) ? (!customSelected_ && tempPresetIdx_ == i)
                                       : customSelected_;
    ...
    if (i < kPresetCount) {
      ...
      if (isLocal)
        ...
        std::snprintf(rowLabel, sizeof(rowLabel), "%s  (UTC+%d)", kPresets[i].label, off);
      else
        std::snprintf(rowLabel, sizeof(rowLabel), "%s  (UTC%d)", kPresets[i].label, off);
    } else {
    ...
    cat->drawText(renderer, rowLabel, r.x + 10, r.y + 4, rowColor, FontStyle::UI);

    // Sub-label for Local: explain DST tracking
    if (isLocal) {
      cat->drawText(renderer, "follows DST automatically",
                    r.x + 10, r.y + 15, themes.textDim, FontStyle::Micro);
    }
  }
  ...
  cat->drawText(renderer, "UTC+/-", menuRect_.x + 4, baseY + 3, labelCol,
                FontStyle::UI);
  cat->drawText(renderer, "Label", menuRect_.x + 4, baseY + customOffsetRect_.h + 9,
                labelCol, FontStyle::UI);

  bool offAct = customSelected_ && offsetActive_;
  bool lblAct = customSelected_ && !offsetActive_;

  customOffsetInput_.render(renderer, fontMgr_,
      customOffsetRect_.x, customOffsetRect_.y,
      customOffsetRect_.w, customOffsetRect_.h,
      FontStyle::UI, 4,
      offAct, true,
      themes.accent, themes.border, themes.text, themes.text, themes.textDim, "-5");

  customLabelInput_.render(renderer, fontMgr_,
      customLabelRect_.x, customLabelRect_.y,
      customLabelRect_.w, customLabelRect_.h,
      FontStyle::UI, 4,
      lblAct, true,
      themes.accent, themes.border, themes.text, themes.text, themes.textDim, "EST");
  ...
  auto drawBtn = [&](const SDL_Rect &r, const char *label, SDL_Color bg) {
    ...
    cat->drawText(renderer, label, r.x + r.w / 2, r.y + r.h / 2,
                  themes.text, FontStyle::SmallBold, true, false, true);
  };
...
bool ClockAuxPanel::onMouseUp(int mx, int my, Uint16 /*mod*/, int /*clicks*/) {
  ...
  if (customSelected_ &&
      mx >= customOffsetRect_.x && mx < customOffsetRect_.x + customOffsetRect_.w &&
      my >= customOffsetRect_.y && my < customOffsetRect_.y + customOffsetRect_.h) {
    offsetActive_ = true;
    customOffsetInput_.setActive(true);
    customLabelInput_.setActive(false);
    return true;
  ...
  if (customSelected_ &&
      mx >= customLabelRect_.x && mx < customLabelRect_.x + customLabelRect_.w &&
      my >= customLabelRect_.y && my < customLabelRect_.y + customLabelRect_.h) {
    offsetActive_ = false;
    customOffsetInput_.setActive(false);
...
bool ClockAuxPanel::onKeyDown(SDL_Keycode key, Uint16 mod) {
  ...
  if (key == SDLK_TAB) {
    ...
    if (offsetActive_) customOffsetInput_.setCursorToEnd();
    else               customLabelInput_.setCursorToEnd();
    return true;
  }

  if (offsetActive_) return customOffsetInput_.onKeyDown(key, mod);
...
bool ClockAuxPanel::onTextInput(const char *text) {
  if (!menuVisible_ || !customSelected_)
    return false;
  if (offsetActive_) return customOffsetInput_.onTextInput(text);
