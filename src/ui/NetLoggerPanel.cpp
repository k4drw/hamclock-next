...
REGISTER_WIDGET("netlogger", "NetLogger", true, false, {
    return std::make_unique<NetLoggerPanel>(0, 0, 0, 0, deps.fontMgr);
})

NetLoggerPanel::NetLoggerPanel(int x, int y, int w, int h, FontManager& fontMgr)
    : Widget(x, y, w, h), fontMgr_(fontMgr) {
}

void NetLoggerPanel::update() {
    data_ = NetLoggerStore::instance()->get();
    
    // Auto-scroll logic if needed, or adjust maxScroll
    if (data_.hasCheckins) {
        maxScroll_ = std::max(0, (int)data_.checkins.size() * rowHeight_ - (height_ - 30));
    } else {
        maxScroll_ = std::max(0, (int)data_.activeNets.size() * rowHeight_ - (height_ - 30));
    }
...
void NetLoggerPanel::renderCheckinList(SDL_Renderer* renderer, const ThemeColors& themes) {
    ...
    SDL_Rect headerRect = {x_ + 5, cy, width_ - 10, rowHeight_};
    SDL_RenderFillRect(renderer, &headerRect);
    
    cat->drawText(renderer, "< Back to Nets", x_ + 10, cy + 5, themes.accent, FontStyle::SmallBold);
    cy += rowHeight_;
...
bool NetLoggerPanel::onMouseUp(int mx, int my, Uint16 mod, int clicks) {
    ...
    if (data_.hasCheckins && !data_.selectedNetName.empty()) {
        if (clickedRow == 0) {
            ...
            NetLoggerStore::instance()->setSelectedNet("", "");
            scrollY_ = 0;
            return true;
        }
    } else {
        if (clickedRow >= 0 && clickedRow < (int)data_.activeNets.size()) {
            ...
            NetLoggerStore::instance()->setSelectedNet(net.serverName, net.netName);
            scrollY_ = 0;
            return true;
        }
    }
