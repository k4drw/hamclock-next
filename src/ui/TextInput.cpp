...
std::string TextInput::getSelectedText() const {
    if (cursorPos_ == selectionAnchor_) return {};
    int start = std::min(cursorPos_, selectionAnchor_);
    int end   = std::max(cursorPos_, selectionAnchor_);
...
void TextInput::render(SDL_Renderer *renderer, FontManager &fontMgr,
                       int fieldX, int fieldY, int fieldW, int fieldH,
    ...
    if (active) {
        ...
        SDL_SetRenderDrawColor(renderer, border.r / 2, border.g / 2, border.b / 2, 255);
    ...
    if (!text_.empty()) {
        ...
        cat->drawText(renderer, text_, fieldX + textPad, fieldY + textPad, color, fieldStyle);
    ...
    if (active) {
        ...
        if ((SDL_GetTicks() / 500) % 2 == 0) {
            ...
            SDL_RenderDrawLine(renderer, cursorX, fieldY + 4, cursorX, fieldY + fieldH - 4);
        }
    }
}
